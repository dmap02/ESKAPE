...
tre = ape::read.tree("~/Desktop/NATURE/Figure4/3_way_masked_adqia_masked.min4.phy.contree")


#let's read in the metadata
mydat = read.csv("~/Desktop/NATURE/Figure4/metadata_colors.csv") %>%
  dplyr::select(., c("rg", "color"))
...
new.labels = read.csv("~/Desktop/NATURE/Figure4/relabel_tree.csv")
...
```{r}
# vcfR
consensus3way = read.vcfR("~/Desktop/NATURE/Figure4/3_way_masked_adqia_masked.vcf.gz", verbose=FALSE) 
...
Fig4
ggsave(Fig4, file="~/Desktop/NATURE/ABX_SAMPLES/Manuscript/Figure4.pdf", device="pdf", width = 7, height = 8)
```

### Supplementary Figure 12B: let's look at within subject vs. between subject SNP distributions
```{r, fig.height=4, fig.width=4}
#read in the vcf, taken as an intersection of GATK, freebayes and bcftools
myvcf = read.vcfR("~/Desktop/NATURE/Figure4/3_way_masked_adqia_masked.vcf.gz")
...
mapping_file = read.csv("~/Desktop/NATURE/Figure4/metadata_colors.csv") %>%
  dplyr::select(., c("rg", "Subject", "Sample", "Body.Site", "room"))
...
SuppFig12B = ggplot(mydf, aes(is.same.subject, value)) + 
...
### Supplementary Figure 12C: make a plot of roomie comparisons
```{r, fig.height=4, fig.width=4}
#now let's annotate whether the sample1 and sample2 are from the same or different subjects
sampleDF1 = subset(sampleDF1, room1 %in% c("room1", "room2")) %>%
  subset(., room2 %in% c("room1", "room2"))
sampleDF1$is.roomie = ifelse(sampleDF1$room1==sampleDF1$room2, "yes", "no")
SuppFig12C = ggplot(sampleDF1, aes(is.roomie, value)) + theme_classic() + stat_compare_means() + geom_boxplot() + 
  stat_summary(fun = "mean",
               geom = "point",
               color = "black",
...
SuppFig12A = ggplot(sameSub, aes(value)) + 
  geom_histogram() + 
  facet_wrap(~Sub1, scales="free_y", ncol=1) + 
  theme_classic() + ylab("Count") + xlab("Tree Distance")

#let's save supplementary figure 12
Fig12C_Left = cowplot::plot_grid(SuppFig12A, labels="A")
Fig12C_Right = cowplot::plot_grid(SuppFig12B, SuppFig12C, ncol= 1, labels=c("B", "C"))
Fig12C = cowplot::plot_grid(Fig12C_Left, Fig12C_Right, ncol=2)
ggsave(Fig12C, file="~/Desktop/NATURE/ABX_SAMPLES/Manuscript/SupplementaryFigure12.png", device="png", width = 8, height = 10)
```
