...
1. mapping_file_2022-08-15_v4.1.csv, the metadata for the shotgun data
...
3. ESKAPE_phyloseq_50percent.rds, a phyloseq object describing abundance of species per sample where at least 50% of genome is covered
4. srst2_take3.txt, output of srst2
5. Cauris_MDRO history.csv, the history of antibiotic resistance mechanisms 
6. ESKAPE_phyloseq_nocoveragethreshold.rds, the phyloseq object where abundance is defined by total unique reads mapping irrespective of breadth of genome coverage
...
This script will generate the fo llowing plots:

1. Figure 2
2. correlation between antibiotic resistance gene depth vs. depth of reads mapping to unique species
...
HighCovDF$HighCov =  ifelse(HighCovDF$HighCov > 0, "High", "Low")
...
newcolours=c("High"= "midnightblue", "Low"= "lightskyblue4" ) 
...
Fig2A = ggplot(FinalDF, aes(as.factor(Unique_ptid), Var1))+ 
  ...
  scale_color_manual(values=newcolours) +
...
Fig2
ggsave(Fig2, file="~/Desktop/NATURE/ABX_SAMPLES/Manuscript/Figure2.pdf", device="pdf", height = 9, width = 7)
```
