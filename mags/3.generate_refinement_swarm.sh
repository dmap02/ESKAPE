...
ls -d $BINNING_DIR/02_megahit_bins/*/metabat2_bins > "$MEGAHIT_OUTPUT_DIR/megahit_metabat2_bins.list"
ls -d $BINNING_DIR/02_megahit_bins/*/maxbin2_bins > "$MEGAHIT_OUTPUT_DIR/megahit_maxbin2_bins.list"
ls -d $BINNING_DIR/02_megahit_bins/*/concoct_bins > "$MEGAHIT_OUTPUT_DIR/megahit_concoct_bins.list"

ls -d $BINNING_DIR/02_spades_bins/*/metabat2_bins > "$SPADES_OUTPUT_DIR/spades_metabat2_bins.list"
ls -d $BINNING_DIR/02_spades_bins/*/maxbin2_bins > "$SPADES_OUTPUT_DIR/spades_maxbin2_bins.list"
ls -d $BINNING_DIR/02_spades_bins/*/concoct_bins > "$SPADES_OUTPUT_DIR/spades_concoct_bins.list"
...
# Generate Swarm files for MegaHit and SPAdes
mapfile -t megahit_refine_out < "$MEGAHIT_OUTPUT_DIR/megahit_REFINEMENT_OUT.list"
mapfile -t spades_refine_out < "$SPADES_OUTPUT_DIR/spades_REFINEMENT_OUT.list"

mapfile -t megahit_metabat2_bins < "$MEGAHIT_OUTPUT_DIR/megahit_metabat2_bins.list"
mapfile -t megahit_maxbin2_bins < "$MEGAHIT_OUTPUT_DIR/megahit_maxbin2_bins.list"
mapfile -t megahit_concoct_bins < "$MEGAHIT_OUTPUT_DIR/megahit_concoct_bins.list"

mapfile -t spades_metabat2_bins < "$SPADES_OUTPUT_DIR/spades_metabat2_bins.list"
mapfile -t spades_maxbin2_bins < "$SPADES_OUTPUT_DIR/spades_maxbin2_bins.list"
mapfile -t spades_concoct_bins < "$SPADES_OUTPUT_DIR/spades_concoct_bins.list"

# Create Swarm file for MegaHit
> "$MEGAHIT_SWARM_FILE"
total=${#megahit_refine_out[*]}
for (( i=0; i<$total; i++ )); do
    output_dir="${megahit_refine_out[i]}"
    echo "bash $REFINE_SCRIPT $output_dir ${megahit_metabat2_bins[i]} ${megahit_maxbin2_bins[i]} ${megahit_concoct_bins[i]} $SIF_FILE" >> "$MEGAHIT_SWARM_FILE"
done

# Create Swarm file for SPAdes
> "$SPADES_SWARM_FILE"
total=${#spades_refine_out[*]}
for (( i=0; i<$total; i++ )); do
    output_dir="${spades_refine_out[i]}"
    echo "bash $REFINE_SCRIPT $output_dir ${spades_metabat2_bins[i]} ${spades_maxbin2_bins[i]} ${spades_concoct_bins[i]} $SIF_FILE" >> "$SPADES_SWARM_FILE"
done
