...
"#FEAF16", "firebrick", "darkgreen", "yellow")
...
Figure  3: a) Histograms of pairwise Average nucleotide identity (ANI) for MAGs recovered from shotgun metagenomic data for each species. Colors correspond to each select species. 
```{r}
...
Fig3A_Alt = ggplot(sameSpecies, aes(ANI, fill=species1.new)) + geom_histogram()+
...
### Panel 3B: Let's plot ANI networks, drawing an edge between nodes only if ANI > 99.9
...
Fig3B = cowplot::plot_grid(p1, p9, p2, p3, p4, p5, p6, p7, p8 , ncol=3, labels="B")
Fig3B
```
...
Fig3C
```
let's get the summary stats for the number of SNPs
...
let's look at the number of SNPs between samples from the same person - acinetobacter

```{r}
map1 = read.csv("~/Desktop/NATURE/mapping_file_2023-07-23.csv") %>%
          dplyr::select(., c("library_id", "Unique_ptid", "site_specific", "Survey_Period"))

ac_snps$library_id = ac_snps$Node
mydf = merge(ac_snps, map1)
mydf$Label = paste0(mydf$Unique_ptid, ";", mydf$site_specific)

p = ggplot(mydf, aes(Survey_Period, `Num of SNPs outside recombinations`)) + 
  geom_violin()  +
  facet_wrap(~Unique_ptid) +
  geom_jitter(aes(color=site_specific), width = 0.5, size=3)
p
```
...
kp_snps$library_id = kp_snps$Node
mydf = merge(kp_snps, map1)
mydf$Label = paste0(mydf$Unique_ptid, ";", mydf$site_specific)

p = ggplot(mydf, aes(Survey_Period, `Num of SNPs outside recombinations`)) + 
  facet_wrap(~Unique_ptid) +
  geom_jitter(aes(color=site_specific), width = 0.5, size=3) + geom_hline(yintercept = 50)
p
```
...
ec_snps$library_id = ec_snps$Node
mydf = merge(ec_snps, map1)
mydf$Label = paste0(mydf$Unique_ptid, ";", mydf$site_specific)
...
ca_snps$library_id = ca_snps$Node
mydf = merge(ca_snps, map1)
mydf$Label = paste0(mydf$Unique_ptid, ";", mydf$site_specific)
...
ac_snps = data.table::fread("~/Desktop/NATURE/Figure3/ac_snps/core.full.per_branch_statistics.csv")
ac_snps$library_id = ac_snps$Node
mydf = merge(ac_snps, map1)
mydf$Label = paste0(mydf$Unique_ptid, ";", mydf$site_specific)
...
df = read.csv("~/Desktop/NATURE/ABX_SAMPLES/goToTree/kleb2_gtree_out/tree.tip.labels_klebsiella.csv")
sequenceType = data.frame(node=df$new.label, ST=df$ST)
rownames(sequenceType) = sequenceType$node


#make the tre
Fig3D = ggtree(tre2) %<+% df +  theme_tree2()+ geom_tiplab(aes(label=new.label, color=ST))+
    scale_color_manual(breaks=unique(df$ST), values=mypal, name="ST", na.value="white")+
  theme(legend.position="none") 
Fig3D
```

### Fig3E: zero snp subjects
...
Fig3E = ggplot(combinedDF, aes(as.factor(subject), species, color=species, group=species)) + 
...
### Generate Figure 3
Figure  3: Skin of nursing home residents is a reservoir for the transmission of ESKAPE pathogens. a) Histograms of pairwise Average nucleotide identity (ANI) for MAGs recovered from shotgun metagenomic data for each species. Colors correspond to each select species. b) Network analysis of pairwise ANI for each species. Each node (large dot) represents a MAG. Edges or lines between nodes are represented if pairwise ANI reaches or exceeds 99.9%. Colors correspond to distinct subjects. For E. coli, S. pettenkoferi, and C. auris unconnected nodes represent cases where one of two pairwise calculations had ANI less than 99.9%. c) Number of SNPs per genome of species defined in upper panel, as determined by a read-based approach, relative to the nearest NCBI neighbor (identified in the single copy marker gene analysis). d) Nursing home residents (subject ID on x-axis) with genomic data consistent with sharing a clonal strain of each species (y-axis). Subjects are ordered from the great left to right based on descending numbers of shared strains. Colors of points and lines are as in panel a. If MAG yielded 0 SNPs outside recombinant regions, with breadth of coverage of the reference genome exceeding 75%, then strains are considered ‘shared’ and marked with large dot on line. Subjects 2, 4, 28 and 48 were roommates, as were subjects 14, 23, 5, and 15. 

```{r, fig.height=14, fig.width=12}
Fig3_Left = cowplot::plot_grid(Fig3A_Alt, Fig3B, ncol=1, labels=c("A", "B"))
Fig3_Middle = cowplot::plot_grid(Fig3D, labels="C")
Fig3_Right= cowplot::plot_grid(Fig3C, labels="D")
Fig3_TOP = cowplot::plot_grid(Fig3_Left , Fig3_Middle, Fig3_Right, ncol=3)
Fig3_Bottom = cowplot::plot_grid(Fig3E, ncol=1, rel_widths = 1, rel_heights = 0.7, labels="E")

Fig3 = plot_grid(Fig3_TOP, Fig3_Bottom ,nrow = 2, rel_heights = c(2, 0.5))
Fig3
ggsave(Fig3, file="~/Desktop/NATURE/ABX_SAMPLES/Manuscript/Figure3.pdf")
```