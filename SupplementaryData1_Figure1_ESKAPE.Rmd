...
```{r}
#define the color palettes we want to use
newcolours=c("Candida auris"= "#BC80BD",
         "Malassezia"= "#BEBADA" ,
        "Malassezia slooffiae"="violet", 
        "s__Pseudomonas aeruginosa"="#C51B7D",
        "s__Proteus mirabilis"="#E9A3C9",
        "s__Providencia stuartii"=  "#FDE0EF"  ,
        "s__Escherichia coli"="#E6F5D0"   ,
        "s__Enterococcus faecalis"="#E6F5D0"  ,
        "s__Acinetobacter baumannii"="#A1D76A" ,
        "s__Morganella morganii"="#8B8000" ,
        "s__Klebsiella pneumoniae" = "#4D9221", 
        "s__Staphylococcus aureus" = "#329D9C" ,
        "s__Corynebacterium striatum"  ="#56C596",
        "s__Corynebacterium aurimucosum_E"="#7BE495" ,
        "s__Staphylococcus pettenkoferi" = "#CFF4D2",
        "Other"="#85C1E9" ) 
...
Supplementary Figure 2: Variation in read mapping based on body site. left) Reads were mapped to the composite MAG collection shown in figure 1A and the % of total microbial reads captured by the species shown in Figure 1A is displayed (y-axis) as a function of body site. total microbial reads captured by the ESKAPE pathogens (y-axis) is plotted as a function of body site (x-axis). Abbreviations include An Peri-anal skin, Ea external auditory canal, Fg fingterips/palm, Ic inguinal crease, N nares, and Tw toe-webs. Differences between means were computed with pairwise t-tests and adjusted for multiple testing using the holm method.

```{r, fig.height=5, fig.width=5}
#again we create our data frame
mydf = Fig1B_alt$data

#now we subset on the eskape pathogens and candida auris
mydf = subset(mydf, group %in% c("Escherichia coli","Enterococcus faecalis",
                                  "Staphylococcus aureus" ,"Acinetobacter baumannii"  ,
                                  "Pseudomonas aeruginosa_A" ,   "Klebsiella pneumoniae" ,
                                  "Pseudomonas aeruginosa",  "Candida auris"   ))

#we generate the per samples averages of the eskape pathogen abundances
averages = doBy::summary_by(Abundance~Label, FUN=sum, data=mydf)

# we generate that map
map = reshape2::colsplit(averages$Label, ";", c("Subject", "Site", "Survey"))
df = data.frame(averages, map)

#we generate survey wise averages estimates
averages1 = subset(df, Survey=="Survey_1") %>%
  na.omit(.)
  averages1 = doBy::summary_by(Abundance.sum~Site, data=averages1, FUN=c(mean, sd, length))
  averages1$Survey = 1
averages2 = subset(df, Survey=="Survey_2") %>%
  na.omit(.)
  averages2 = doBy::summary_by(Abundance.sum~Site, data=averages2, FUN=c(mean, sd, length))
  averages2$Survey = 2
averages3 = subset(df, Survey=="Survey_3") %>%
  na.omit(.)
  averages3 = doBy::summary_by(Abundance.sum~Site, data=averages3, FUN=c(mean, sd, length))
  averages3$Survey = 3
myaverages = data.frame(rbind(averages1, averages2, averages3))



#and we plot the relative abundance captured by the eskape pathogens, averaging across surveys
Sfig2 = ggplot(myaverages, aes(Site, Abundance.sum.mean)) + geom_boxplot() + geom_point() + 
  theme_classic() + 
...
let's see how many MAGs we recover per sample, for these select species. 25 samples had more than 5 of these species.
...
fm = subset(PrevBySample, prev.sum >= 5)
...
Figure1 = cowplot::plot_grid(Fig1A, Fig1B_alt, ncol = 1, align = "v", rel_heights = c(1, 0.95), 
  labels="auto")
Figure1
ggsave(Figure1, file="~/Desktop/NATURE/ABX_SAMPLES/Manuscript/Figure1.pdf", device="pdf", width = 7.5, height = 6)
...
### supplementary figure 3: what is the breadth and depth of coverage for each of these species
...
SupplementaryFigure3A = ggplot(depth, aes(species, log(value), fill=variable)) + geom_boxplot() +
  theme_classic() +
  xlab("Depth of Coverage") + 
    scale_fill_manual(values=c("#FFD92F")) + 
    coord_flip()
...
#get summary stats for the depth of coverage
```{r}
...
plot the depth and breadth of coverage stats
```{r, fig.width=6, fig.height=5}

# Initialize ggplot with data
SupplementaryFigure3A<- ggplot(
  depth, 
  aes(x = species, y = average, ymin = CI_lower, ymax = CI_upper)) +
  geom_pointrange() + geom_point() +ylim(0, 300) + 
...
SupplementaryFigure3B<- ggplot(
  coverage, 
  aes(x = species, y = average, ymin = CI_lower, ymax = CI_upper)) +
  geom_pointrange() + geom_point() +ylim(0, 100) + 
  coord_flip()  +
  theme_classic()


grid.arrange(SupplementaryFigure3A, SupplementaryFigure3B, ncol=2)
...
get the summary statistics for the metagenomics sequencing runs
```{r}
df = read.csv("~/Desktop/NATURE/Figure1/SupplementaryTable1_metagenomesummaries.csv") %>%
  subset(., is.control !="yes")
summary(as.numeric(df$TOTAL.READS), na.rm=TRUE)
```
